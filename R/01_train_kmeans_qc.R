...
#'
#' @param mat A transposed numeric matrix or data frame with features in columns and samples in rows.
...
train_kmeans_qc <- function(mat, k.max = 10, nstart = 25, B = 100, seed = 123) {
  ...
  # QC 1 : Elbow (WSS)
  # ------------------------------------------------------------
  p_elbow <- fviz_nbclust(mat, kmeans, method = "wss", k.max = k.max) +
    ggtitle("Elbow")

  # Extraction automatique du elbow
  wss <- vapply(1:k.max, function(k) {
    kmeans(mat, centers = k, nstart = nstart)$tot.withinss
  }, numeric(1))
  ...
  # QC 2 : Silhouette
  # ------------------------------------------------------------
  sil <- fviz_nbclust(mat, kmeans, method = "silhouette", k.max = k.max)
  ...
  # ------------------------------------------------------------
  set.seed(seed)
  gap_stat <- clusGap(mat, FUN = kmeans, nstart = nstart,
                      K.max = k.max, B = B)
  ...
  # ------------------------------------------------------------
  set.seed(seed)
  km_final <- kmeans(mat, centers = optimal_k, nstart = nstart)
